...
class FlaurologicalDriver:
    ...
    def run(self, url: str, save_mode=False) -> None:
        ...
        :param save_mode: <bool> default=False.
        If true then it will run the save_pages method
        :return: None
        """
        if save_mode:
            self.save_pages(url)
    ...
    def _specific_first_page(self) -> None:
        ...
        """
        data = {}
        data['session-title'] = self.__xpath_text(
            '//*[@id="FUS1808"]/tbody/tr[10]/td[3]/div[1]'
        )
        data['speakerType'] = self.__xpath_text(
            '//*[@id="FUS1808"]/tbody/tr[10]/td[3]/div[2]/div[1]'
        )
        data['speakerDetails'] = {}
        elem = self.__xpath_text(
            '//*[@id="FUS1808"]/tbody/tr[10]/td[3]/div[2]/div[2]'
        )
        (data['speakerDetails']['name'],
         data['speakerDetails']['Workplace']) = elem.split('\n')
    ...
    def _gen_data(self, element) -> dict:
        ...
        """
        data = {}
        data['session-title'] = element.find_elements(
            By.TAG_NAME, 'td'
        )[2].find_elements(By.TAG_NAME, 'div')[0].text
        data['speakerType'] = element.find_elements(
            By.TAG_NAME, 'td'
        )[2].find_elements(
            By.TAG_NAME, 'div'
        )[1].find_elements(By.TAG_NAME, 'div')[0].text
        data['speakerDetails'] = {}
        elem = element.find_elements(
            By.TAG_NAME, 'td'
        )[2].find_elements(
            By.TAG_NAME, 'div'
        )[1].find_elements(By.TAG_NAME, 'div')[1].text
        elem = elem.split('\n')
        if len(elem) > 2:
            names = []
            workplaces = []
            ...
            (data['speakerDetails']['names'],
             data['speakerDetails']['Workplaces']) = names, workplaces
        else:
            (data['speakerDetails']['names'],
             data['speakerDetails']['Workplaces']) = [elem[0]], [elem[-1]]
        return data
    ...
    def __download_page(self, i: int) -> None:
        ...
        :param i: <int> number of the page.
        It will be the specific name of '.html' document
        :return: None
        """
